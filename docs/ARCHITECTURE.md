...
> **Última actualización:** 2026-09-25 (ADR #41: liquidación = fotografía sellada del libro de cajas; atribución por zona/cobrador sellada en cada asiento; caja de ruta como cuenta del cobrador; rendición = corte con orden total por caja; liquidación por períodos contiguos. Plan de control de campo completo (Fases 1–7)).
...
| 37 | **Los webhooks públicos fallan CERRADO + authZ por rol en TODO endpoint del plano de datos** ([SECURITY_AUDIT.md](SECURITY_AUDIT.md)): el webhook de WhatsApp rechaza (403) el evento cuando no puede probar su autenticidad —sin `phone_number_id`, canal desconocido o canal sin App Secret—, igual que ya hacían PicPay y Mercado Pago; y ningún endpoint autenticado se queda solo con `requireTenant` (otorgar crédito → `requireReviewer`; abonar/leer cartera → `DATA_PLANE_ROLES` + alcance de `collector_client` para el COLLECTOR; conciliar → `MANAGER_ROLES`). El saneado del `audit_log` pasa a coincidencia por **subcadena** | "no perder mensajes durante la configuración inicial" convertía el webhook en una puerta abierta a la originación y a los pagos: la disponibilidad de un canal mal configurado no puede costar la integridad del resto. La matriz de permisos del cliente ([authorization.ts](../apps/mobile/src/core/auth/authorization.ts)) ya asumía estas reglas — el servidor era el que no las imponía, así que la autoridad real vivía en la UI. En `audit_log` (append-only) una lista de nombres EXACTOS deja pasar cada campo nuevo, y un secreto filtrado ahí es irreversible | ✅ |
| 38 | **Un documento KYC puede constar de VARIOS archivos + el protocolo se serializa por solicitud** (`credit_application_document_file`, `credit_document_requirement.expected_files`, `CreditApplicationRepository.withActiveApplicationLocked`): el catálogo declara cuántos archivos compone cada documento (2 = "ambos lados"); el agregado lleva `expectedFiles`/`receivedFiles` y solo pasa a `VALIDATED` al reunirlos todos (`RECEIVED` entre medias). Todo el tramo leer-decidir-registrar corre en UNA transacción con `SELECT ... FOR UPDATE` sobre la solicitud, y los intentos fallidos viven en el agregado (`mismatch_attempts`, reiniciado al validar) en vez de contarse sobre el historial de `document_extraction` | pedir "ambos lados" con un solo cupo de archivo hacía que la segunda foto se juzgara contra el documento SIGUIENTE; y como WhatsApp entrega cada foto de un álbum como un webhook independiente, las dos leían el mismo estado y una gastaba un intento sin culpa del solicitante. El cerrojo por solicitud es el mínimo que ordena los mensajes de un mismo remitente sin afectar a los demás; obliga a subir el `max` del pool (`PG_POOL_MAX`) porque los adaptadores anidados abren su propia conexión | ✅ |
| 39 | **Consola de comunicaciones de WhatsApp: los fallos técnicos se PERSISTEN y la cola se puede depurar** (`conversation_failure`, `apps/api/src/whatsapp/conversation-scope.sql.ts`, `domain/conversations/conversation-outcome.ts`): el `catch` del webhook, que hasta ahora solo escribía en el log del proceso, registra además el mensaje que no se pudo atender (canal, teléfono, zona, etapa derivada del tipo, error truncado). Con eso la bandeja clasifica cada conversación en un **desenlace** derivado (`ONLY_INQUIRY` · `TECHNICAL_FAILURE` · `INCOMPLETE` · `PENDING_APPROVAL` · `APPROVED` · `REJECTED`), cuya precedencia vive en el dominio puro (`classifyConversationOutcome`, con pruebas) y se transcribe a un `CASE` en el read model **solo** para poder filtrar y agregar en la BD. Un único CTE (`enrichedConversationsCte`) alimenta listado, estadística y purga, de modo que la limpieza masiva no pueda borrar algo distinto de lo que el operador está viendo. El transcript recupera el `DELETE` para el rol `app` (el `UPDATE` sigue revocado) | un fallo que solo existe en el log del proceso es invisible para la operación: no había forma de distinguir a quien abandonó de quien se quedó a medias porque el sistema falló, que es justamente el caso recuperable. La regla de clasificación es negocio (decide a quién se llama), así que no puede vivir únicamente en SQL. Sobre el borrado: se cede el `DELETE` porque una bandeja que solo crece deja de usarse (y el titular tiene derecho de supresión), pero **no** el `UPDATE` —reescribir un mensaje es lo que falsearía el rastro—, cada depuración queda en `audit_log` (append-only) con actor y filtros, y la conversación que respalda un crédito APROBADO no se borra nunca: es la evidencia de cómo se originó | ✅ |
| 40 | **Canal Telegram junto a WhatsApp, por zona y habilitable por tenant** (`apps/api/src/telegram`, `apps/api/src/messaging`, `domain/conversations/{messaging-channel,telegram-contact,proactive-channel}.ts`, `tenant_config.messaging_channels`, migraciones 0055–0056; plan en [PLAN_TELEGRAM_CANAL.md](PLAN_TELEGRAM_CANAL.md)): Telegram es **otro driver detrás de los mismos puertos** (`OutboundTextSender`, `MediaDownloader`, `TenantResolver`), elegido por el prefijo del `channelId` (`tg:<bot_id>`; WhatsApp sin prefijo, sin migrar datos). **La identidad sigue siendo el teléfono**: un chat no entra al flujo hasta compartir SU contacto con el botón nativo (`contact.user_id === from.id`), así que la misma persona por ambos canales es el mismo solicitante/deudor. Un bot por zona (`telegram_channel`, token cifrado) con webhook por bot en URL opaca + secret token (falla cerrado), registrado solo por la API (`setWebhook`). Los avisos por iniciativa propia usan el **canal alcanzable** (`chooseProactiveChannel`: último usado → guardado → zona según preferencia → heredado) | tablas propias en vez de generalizar `whatsapp_channel` (credenciales heterogéneas, funciones SECURITY DEFINER y datos productivos intactos); `tg:<user_id>` como identidad rompería la unicidad de la solicitud por persona, el cruce con `borrower.phone`, el antifraude por DDD y la bandeja | ✅ |
| 41 | **Liquidación = fotografía sellada del libro de cajas; zona y cobrador sellados en cada asiento; caja de ruta = cuenta del cobrador** (plan en [PLAN_CONTROL_CAMPO_Y_LIQUIDACION.md](PLAN_CONTROL_CAMPO_Y_LIQUIDACION.md); Fase 1: `domain/cash/ledger-attribution.ts`, `domain/credit/portfolio/principal-interest-split.ts`, `api/cash/cash-ledger.ts#attributionFor`, `api/payments/allocation-writer.ts`): la liquidación por período no calcula saldos, agrega `cash_transaction` en `[inicio, fin)`. Cada asiento **sella** `zone_id` (la del crédito/gasto de origen, o la de la caja) y `collector_id` (dueño de la caja de ruta) al postearse, estable ante reasignaciones. Una zona usa las cajas del tenant, las suyas y las de sus ancestros (`canZoneUseBox`, 409 `BOX_NOT_USABLE_BY_ZONE`). El cobro en efectivo postea `PAYMENT_IN` a la caja de ruta de quien cobra (409 `NO_ROUTE_CASH_BOX` sin caja) y el otorgamiento directo debita su caja (`fundingCashBoxId`), como la aprobación. Cada abono persiste su desglose capital/interés proporcional calculado sobre lo acumulado (Σ exacta). **Rendición (Fase 2)** = corte de la caja de ruta (`collector_remittance`, `domain/cash/remittance.ts`): el cobrador declara, el coordinador cuenta y recibe (`TRANSFER` ruta → oficina) y lo no entregado queda como saldo = **deuda arrastrada**, cerrable solo por el ADMIN con `DEBT_CLOSURE` (`PAYROLL` / `WRITE_OFF`). **Orden total por caja**: todo asiento a una caja se inserta bajo su advisory lock y `cash_transaction.created_at` = `clock_timestamp()`, de modo que "posterior al corte" coincide con el orden de los candados (ningún cobro concurrente se pierde) y el corte se compara en SQL (microsegundos) | reintroducir un escalar de caja paralelo fue la causa del retiro de la liquidación en julio; una tabla de deuda separada sería un segundo número que puede divergir; atribuir por caja falla porque las zonas hijas comparten las cajas del padre; recalcular el desglose en la liquidación haría que la foto dependiera de datos mutables; `created_at = now()` (inicio de tx) permitía que un cobro que confirma después del corte quedara fechado antes y se perdiera | ✅ Fases 1–7 (pendiente despliegue) |
...
> **Hallazgos de seguridad abiertos:** el inventario vivo (severidad, estado, remediación) está en
> [SECURITY_AUDIT.md](SECURITY_AUDIT.md). Los ítems de esta tabla no lo duplican; solo se listan
> aquí los que además son deuda arquitectónica.

| Ítem | Detalle | Acción sugerida |
|---|---|---|
| 🔴 **RLS faltante en `credit_document_requirement`** (aislamiento, [§8](#8-multitenancy-y-seguridad-rls)) | 44 de 45 tablas llevan `ENABLE`+`FORCE`; esta lleva `tenant_id` pero **ninguna política** (SECURITY_AUDIT #4) | migración con el bloque `ENABLE/FORCE/POLICY` estándar — o `DROP` si `required_document_catalog` ya la reemplazó |
| 🔴 **Sesión no revocable** (seguridad, [§3.7](#37-atributos-de-calidad-del-sistema--ilities)) | `refresh` reemite copiando rol/zonas del token viejo sin releer `app_user`: desactivar un usuario no surte efecto en 30 días (SECURITY_AUDIT #6) | releer el usuario (activo+rol+zonas) al refrescar; `jti` + revocación después |
| **Sin rate limiting** (disponibilidad) | `/auth/login` admite fuerza bruta ilimitada; `scrypt` hace cada intento costoso de servir (SECURITY_AUDIT #8) | `@nestjs/throttler` (dependencia a autorizar) o `rate_limit` en el Caddyfile |

...
import { useSecureFileUrl } from "@/core/api/secure-file";

type Props = {
  /** Ruta del API que sirve el archivo descifrado; null = cerrado. */
  path: string | null;
  title: string;
  onClose: () => void;
};
...
/**
 * Visor de un comprobante sensible (pago, gasto…). Descarga el binario descifrado (autenticado)
 * como objectURL y lo muestra; permite "ampliar" (zoom básico sin librerías: imagen grande dentro
 * de scroll horizontal y vertical para hacer pan). Libera el objectURL al cerrar (no deja
 * evidencia/PII en memoria).
 */
export function SecureFileViewer({ path, title, onClose }: Props) {
  const { t } = useT();
  const query = useSecureFileUrl(path);
  ...
  return (
    <Modal visible={path != null} onClose={onClose} title={title}>
      <View className="p-4">

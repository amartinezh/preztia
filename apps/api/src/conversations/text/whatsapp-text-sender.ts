...
export class WhatsappTextSender implements OutboundTextSender {
  ...
  async sendText(to: OutboundRecipient, body: string): Promise<void> {
    ...
    if (!token) {
      ...
      this.logger.warn(`WHATSAPP_ACCESS_TOKEN no configurado; respuesta NO enviada a ${to.recipient}: ${body}`);
      return;
    }
    ...
    if (!res.ok) {
      throw new Error(`WhatsApp Send API respondió ${res.status}: ${await res.text()}`);
    }
  }
}
